...
void logwriter(string q) {
  if (q[17] == 'M' && q[18] == 'o' && q[19] == 'h') {
    if (q[23] == '8') {
    ofstream a;
    a.open("logfiles/Mohn8.txt");
    if (a.is_open()) {
    a << q[0]<<q[4]<<q[5]<<endl;
    a << q[7]<<q[11]<<q[12]<<endl;
    a.close(); 
    }
    else
    cout<< "logwriter: error"<<endl;
    }
  } 
}
string get_status(string b, string f, string m) {
        ifstream log;
	string file = b + f + ".txt";
	log.open(("logfiles/" + file).c_str());
	if (log.is_open()) {
	   if (m == "w") {
		string w;
		log>> w;
...
int main() {
	...
	f << "<h2 id=\"Mohn\">Mohn</h2>" << endl;
        ...
        f << "<h4>Washer 1- " << WMoh8<<"</h4>" << endl;
	f << "<h4>Dryer 1- "<< DMoh8<<"</h4>" << endl;
