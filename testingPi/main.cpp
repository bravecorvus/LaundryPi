...
int main() {
	...
	washerhardware.readAnalIn("firstfiveEx1.txt"); //read the dataset for real time comparison
	// Vibration dryerhardware(dryer);
	cout << "Dataset read in successfully." << endl;
	...
	for(;;) {
		...
		if( (washerhardware.getVecSize()) == washerhardware.tempVecSize + 5000){
			washerhardware.HugosStatisticalAnalysis();
			washerhardware.tempVecSize= washerhardware.getVecSize();
		}
